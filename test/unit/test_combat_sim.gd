...
func test_wave_fill_tracks_monsters_killed() -> void:
	# The bar now steps with MONSTERS KILLED / total, not a damage/time blend.
	CombatSim.follow_mode = false
	CombatSim._wave_kind = "normal"
	CombatSim._wave_count = 5
	CombatSim._kills_emitted = 0
	assert_eq(CombatSim.wave_fill(), 0.0, "no kills → 0%")
	CombatSim._kills_emitted = 2
	assert_almost_eq(CombatSim.wave_fill(), 40.0, 0.0001, "2 of 5 monsters → 40%")
	CombatSim._kills_emitted = 5
	assert_eq(CombatSim.wave_fill(), 100.0, "all monsters → 100%")
	CombatSim._reset_wave()
	assert_eq(CombatSim.wave_fill(), 0.0, "fresh wave starts at 0%")


func test_normal_wave_clears_on_damage_with_no_time_floor() -> void:
	# Wave length is monster-count x time-to-kill now — there is NO minimum
	# duration, so a high-DPS party clears a normal wave the tick its damage
	# crosses the pool (offline agrees: wave_time = ceil(pool/dps)).
	GameState.reset_to_defaults()
	...
	CombatSim.party_dps = CombatSim.wave_pool * CombatSim.TICK_RATE  # one tick = full pool
	var start_wave := CombatSim.wave
	CombatSim._tick()
	assert_ne(CombatSim.wave, start_wave, "no time floor: a one-tick-DPS party clears immediately")


func test_offline_normal_wave_is_dps_driven() -> void:
	# With no minimum duration, an over-powered party clears many normal waves in
	# a short window offline (each ~1 tick), not one-per-fixed-window.
	GameState.reset_to_defaults()
	GameState.player_level = 47
	CombatSim.act = 1
	CombatSim.stage = 1
	...
	var d := CombatSim.simulate_offline(2)
	assert_gt(int(d["waves"]), 1, "no min duration: an OP party clears several waves in 2s")
